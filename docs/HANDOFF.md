...
**H7. NPC 防代演三件套 + 杂修（傍晚续）**
- **NPC/旁白模式**（`DramaChar.npc` + CharEditor 开关）：轮到 TA 说话时，代码把边界文本追加到**请求最后一条 user 消息末尾**（离生成点最近＝最强势），点名保护其他所有成员——她的 NPC 老代演顾荒/苏禾，卡里骂都没用，这是物理围栏。配套给了她卡指令模板 + @Depth 世界书条目写法（见对话，卡指令的坑：让NPC"描写人物状态过渡"会跟"禁演主角"打架）。
- 消息 ✏️ 改写 → 复用 `BigTextEditor` 全屏编辑（保存按最新消息列表改防覆盖）。
...
- ⏭ 角色库的 LibCharEditor 没同步这套（折叠/全屏/指定模型），她要的话照抄 CharEditor。
