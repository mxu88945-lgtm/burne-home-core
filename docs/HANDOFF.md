...
3. 聊天增强批 ← 🚧 进行中：
   ...
   - 截图选段导出 ✅：聊天头部「截图」进选择模式→点选消息→「生成长图」。html2canvas（动态懒加载）
     截离屏导出容器(`exportRef`)，出图后走 lightbox 预览（iOS 长按存相册）。颜色是 hex/rgba 无 oklch，兼容 OK。
   - ⏭ 生图（＋ 菜单加「生成图片」，下一轮做，需先定文生图服务/模型）；＋ 菜单可扩展「表情包」。
